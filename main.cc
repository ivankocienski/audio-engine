...
int main( int arch, char** argv ) {
  ...
  try {
...

    {
      ...
      ap.beep( audio.oscillator(AudioService::WF_SINE), 1.0, 1000, 500 );
      ap.beep( audio.oscillator(AudioService::WF_SILENCE), 1.0, 1000, 100 );

      ap.beep( audio.oscillator(AudioService::WF_SQUARE), 1.0, 1000, 500 );
      ap.beep( audio.oscillator(AudioService::WF_SILENCE), 1.0, 1000, 100 );

      ap.beep( audio.oscillator(AudioService::WF_NOISE), 1.0, 1000, 500 );
      ap.beep( audio.oscillator(AudioService::WF_SILENCE), 1.0, 1000, 100 );

      ap.beep( audio.oscillator(AudioService::WF_SAWTOOTH), 1.0, 1000, 500 );
      ap.beep( audio.oscillator(AudioService::WF_SILENCE), 1.0, 1000, 100 );

      ap.beep( audio.oscillator(AudioService::WF_TRIANGLE), 1.0, 1000, 500 );
      ap.beep( audio.oscillator(AudioService::WF_SILENCE), 1.0, 1000, 100 );

      audio.set_pattern( 4, ap );
    }
    ...
    while( window.active() ) {
      ...
      if( should_paint(0) ) window.puts( 20, 30, "Sound 1 - volume" );
      if( should_paint(1) ) window.puts( 20, 50, "Sound 2 - pitch" );
      if( should_paint(2) ) window.puts( 20, 70, "Sound 3 - waveform" );
      ...
      switch( window.inkey() ) {
        ...
        case Window::K_ENTER:
          ...
          switch( selected ) {
            case 0: audio.play(0, 2); break;
            case 1: audio.play(0, 3); break;
            case 2: audio.play(0, 4); break;
